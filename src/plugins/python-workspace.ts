...
export class PythonWorkspace extends WorkspacePlugin<Package> {
  ...
  protected bumpVersion(pkg: Package): Version {
    ...
    // If the package has its own version (from pyproject.toml / setup.cfg / setup.py),
    // always bump from that. extraVersions stores the OLD version of a dependency as
    // recorded in another package's pyproject.toml — returning it directly would produce
    // a stale version instead of the correctly-incremented one, causing the manifest to
    // not update (e.g. preprocess stays at 1.1.2 instead of advancing to 1.1.4).
    if (pkg.version) {
  ...
  protected updateCandidate(
    ...
    // Always sync newVersion into the ReleasePleaseManifest updater.
    // The base class creates this updater with the old version from the manifest;
    // without this fix the manifest entry for the package never gets updated even
    // though every other file (pyproject.toml, setup.cfg, …) is correctly bumped.
    for (const update of existingCandidate.pullRequest.updates) {
      if (update.updater instanceof ReleasePleaseManifest) {
        (update.updater as any).versionsMap.set(pkg.path, newVersion);
        this.logger.info(`Synced manifest version for ${pkg.path} -> ${newVersion}`);
      }
    }
